...
def build(
    ...
    with _workspace(options) as workspace:
        ...
        supported_partitions, unsupported_partitions = _partition_support(extracted)
...
def list_partitions(
    ...
    with _workspace(options) as workspace:
        ...
        return [
            ListedPartition(name=path.stem, supported=path.stem in supported)
...
def _partition_support(extracted: list[Path]) -> tuple[set[str], set[str]]:
    ...
    raw_only_exact = {"boot", "init_boot", "vendor_boot", "dtbo", "recovery"}
